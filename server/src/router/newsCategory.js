...
Router.get("/allNewsCategory", verifyToken, async (req, res) => {
    const result = await find_Emp_by_name_row("id",req.userId)
    if(result)  {
        try {
            ...
            if (!newsCategorys) {
                ...
                return res
                    .status(200)
                    .json({ success: true, newsCategorys });
            }
        } catch (error) {
            return res
                .status(500)
...
Router.post("/addNewsCategory", verifyToken, async (req, res) => {
    if (req.role.id === 1 || req.role.id === 3) {
        ...
        if (
            ...
            const nameNewsCategoryRe = await find_by_name_row_newsCategory("nameNewsCategory", nameNewsCategory);
            if (nameNewsCategoryRe.length > 0) {
                ...
                try {
                    ...
                    const newNewsCategoryRe = await InsertNewsCategory(newNewsCategoryItem);
...
Router.put("/updateNewsCategory/:id", verifyToken, async (req, res) => {
    if (req.role.id === 1 || req.role.id === 3) {
        ...
        if (
            ...
            const nameNewsCategoryRe = await find_by_name_row_newsCategory("nameNewsCategory", req.params.id);
            if (nameNewsCategoryRe.length > 0) {
                res.status(400).json({
                    success: false,
                    message: "Ngành hàng đã tồn tại",
                ...
                try {
                    ...
                    const newNewsCategoryRe = await UpdateNewsCategory(newNewsCategoryItem,req.params.id);
                    if(newNewsCategoryRe){
                        res.status(200).json({
                            success:true,
